...
// 검색 화면 토글 기능
const searchView = document.querySelector('[data-search-view]'); //속성요소를 선택하는 방법
...
searchField.addEventListener('input', function () {
  ...
  if (searchField.value) {
    searchTimeout = setTimeout(() => {
      if (!searchField.value) return; // 글씨가 없을떄 기능 멈춤
...
const currentLocationBtn = document.querySelector('[data-container]');
  ...
  fetchData(url.currentWeather(lat, lon), function (data) {
    ...
    <h2 class="title-2 card-title">Now</h2>
              ...
              <ul class="meta-list">
                ...
                <li class="meta-item">
                  <span class="m-icon">location_on</span>
                  <p class="title-3 meta-text">${name}, ${country} GB</p>
                </li>
              </ul>
  `;
    ...
    fetchData(url.forecast(lat, lon), function (forecast) {
      ...
      for (let i = 7; i < forecastList.length; i += 8) {
        ...
        <img src="images/weather_icons/${icon}.png" alt="${description}"
          class="weather-icon">
    ...
    //대기질 정보
    fetchData(url.airPollution(la, lon), function (apData) {
      const [
        {
          main: { aqi },
